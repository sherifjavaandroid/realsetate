...
abstract class PsDao<T extends PsObject<T>> {
  ...
  Future<dynamic> getOneWithSubscription(
    ...
    final dynamic subscription = dao
        .query(finder: finder)
        .onSnapshots(await db)
        .listen((dynamic recordSnapshots) {
      T? result;
  ...
  Future<dynamic> getAllWithSubscription(
      ...
      Function? onDataUpdated}) async {
    finder ??= Finder(
        sortOrders: <SortOrder<Object?>>[SortOrder<Object?>(sortingKey, true)]);

    // final dynamic query = dao.query(finder: finder);
    final dynamic subscription = dao
        .query(finder: finder)
        .onSnapshots(await db)
        .listen((dynamic recordSnapshots2) {
      final List<T> resultList = <T>[];
  ...
  Future<dynamic> getAllWithSubscriptionByJoin(
    ...
    final PsResource<List<PsObject<dynamic>>> dataList = await mapDao.getAll(
        finder: Finder(sortOrders: <SortOrder<Object?>>[
      SortOrder<Object?>('sorting', true)
    ]));
  ...
  Future<PsResource<List<T>>> getAll(
      ...
      PsStatus status = PsStatus.SUCCESS,
      String message = '',
      List<SortOrder<Object?>>? sortOrderList}) async {

    // finder ??= Finder(sortOrders: <SortOrder>[SortOrder(sortingKey, true)]);
    if (finder != null) {
      if (sortOrderList == null || sortOrderList.isEmpty) {
        sortOrderList = <SortOrder<Object?>>[
          SortOrder<Object?>(sortingKey, true)
        ];
      } else {
        sortOrderList.add(SortOrder<Object?>(sortingKey, true));
      }
      finder.sortOrders = sortOrderList;
    } else {
      finder ??= Finder(sortOrders: <SortOrder<Object?>>[
  ...
  Future<PsResource<List<T>>>
      getAllByJoin<K extends PsMapObject<dynamic, dynamic>>(
  ...
  Future<PsResource<List<T?>>>
      getAllDataListWithFilterId<K extends PsMapObject<dynamic, dynamic>>(
          String filterId,
          String filterIdKey,
  ...
  Future<PsResource<List<T>>>
      getAllByMap<K extends PsMapObject<dynamic, dynamic>>(
          String primaryKey,
          String mapKey,
          String paramKey,
          PsDao<PsObject<dynamic>> mapDao,
